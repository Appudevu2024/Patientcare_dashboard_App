...
export default function AddPrescription({ id: propId, onClose }) {
  ...
  if (!patient) return <div className="dark:text-white">Patient not found...</div>;

  return (
    <div className="flex items-center justify-center bg-gray-50 dark:bg-gray-900 text-black dark:text-white min-h-screen">
      <div className="p-6 rounded-lg shadow-lg w-full max-w-2xl bg-white dark:bg-gray-800">
        <h2 className="text-2xl font-bold text-[#0967C2] dark:text-blue-400 mb-4">Patient Info</h2>

        <div className="space-y-2 mb-6">
          <p className="text-lg dark:text-blue-300"><strong>Name:</strong> {patient.name}</p>
          <p className="text-lg dark:text-blue-300"><strong>Contact:</strong> {patient.contact}</p>
          <p className="text-lg dark:text-blue-300"><strong>DOB:</strong> {new Date(patient.dateOfBirth).toLocaleDateString('en-GB')}</p>
          <p className="text-lg dark:text-blue-300"><strong>Date:</strong> {patient?.appointment?.date ? new Date(patient.appointment.date).toLocaleDateString('en-GB') : 'N/A'}</p>

          <h3 className="text-lg font-semibold underline text-[#0967C2] dark:text-blue-400">Vital details</h3>
          <p><strong>Height:</strong> {patient?.vitals?.height || 'N/A'} cm</p>
          <p><strong>Weight:</strong> {patient?.vitals?.weight || 'N/A'} kg</p>
          <p><strong>BP:</strong> {patient?.vitals?.bloodPressure || 'N/A'} mm Hg</p>
          <p><strong>Temp:</strong> {patient?.vitals?.temperature || 'N/A'} °F</p>
        ...
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-[#0967C2] dark:text-blue-400 underline mb-2">Previous Prescriptions</h3>
          {patient.prescriptions?.length > 0 ? (
            patient.prescriptions.map((pres, index) => (
              <p key={index} className="mb-1 text-gray-700 dark:text-gray-300">
                <strong>#{index + 1}:</strong> {pres.prescription}
              </p>
            ))
          ) : (
            <p className="text-[#0967C2] dark:text-blue-400">No prescriptions available.</p>
          )}
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block">
            <span className="text-lg font-semibold text-[#0967C2] dark:text-blue-400">New Prescription</span>
            <textarea
              rows="4"
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              className="bg-[#0967C2] hover:bg-blue-700 dark:hover:bg-blue-800 text-white px-4 py-2 rounded"
            >
              Submit Prescription
            </button>
            <button
              type="button"
              onClick={handleCancel}
              className="bg-red-500 hover:bg-gray-600 dark:hover:bg-red-700 text-white px-4 py-2 rounded"
            >
              Cancel
            </button>
          </div>
